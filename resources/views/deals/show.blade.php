<x-app-layout>
    ...
    <div class="max-w-6xl mx-auto py-6 space-y-6">
        ...
        <div class="bg-white shadow rounded-lg p-6">
            ...
            <p class="text-gray-600 mb-2">Client: <span class="font-medium">{{ $deal->client->name }}</span></p>
            <p class="text-gray-600 mb-2">Amount: ${{ number_format($deal->amount, 2) }}</p>
            <p class="text-gray-600 mb-2">Status: <span class="font-semibold">{{ $deal->status }}</span></p>
            <p class="text-gray-600 mb-2">Deadline: {{ $deal->deadline ? \Carbon\Carbon::parse($deal->deadline)->format('d M, Y') : '—' }}</p>
            <p class="text-gray-600 mb-2">Last Updated: {{ $deal->updated_at->diffForHumans() }}</p>
            <div class="flex space-x-4 mt-4">
                <a href="{{ route('deals.edit', $deal) }}" class="px-4 py-2 bg-blue-600 text-white rounded-md">Edit</a>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            @foreach ($statuses as $status)
                <div class="bg-white shadow rounded-lg p-4">
                    ...
                    @foreach ($deal->tasks->where('status', $status) as $task)
